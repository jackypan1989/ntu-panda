...
import ntu.im.bilab.panda.database.Entry;
...
public class ApplicabilityIntegrity {
	...
	public int NoClaims(){
		...
		Count_Dep_Claim(str); //differentiate the dependent claim（we can speculate the indep. claims from dependent one）
	...
	{
		int counter=0;
		for (int i=1;i<Claims.length;i++){ //row 0 isn't claim description
	...
	public class FetchMajorOffice {
		...
		public FetchMajorOffice(String PNO){
		    ...
		    try {
		      URL url = new URL("http://patft.uspto.gov/netacgi/nph-Parser?Sect1=PTO1&Sect2=HITOFF&d=PALL&p=1&u=%2Fnetahtml%2FPTO%2Fsrchnum.htm&r=1&f=G&l=50&s1="+PNO+".PN.&OS=PN/"+PNO+"&RS=PN/"+PNO);
		    ...
		    if(str.indexOf("Foreign Patent Documents")>0){
		       document = document.delete(0,document.indexOf("Foreign Patent Documents")-1);//get rid of unimportant part
		       //System.out.println(document.toString());
		       str = document.substring(document.indexOf("<TABLE "),document.indexOf("</TABLE>")).toString();//get entire table to count Major office
		    ...
		    //}
		    document.setLength(0); //clean stringbuilder to avoid out of memory problem
	...
	public class FetchTransAss {
		...
		public FetchTransAss(String PNO){
			...
			StringBuilder document= new StringBuilder();
		    try {
		      URL url = new URL("http://assignments.uspto.gov/assignments/q?db=pat&qt=pat&reel=&frame=&pat="+PatentNo+"&pub=&asnr=&asnri=&asne=&asnei=&asns=");
		      URLConnection conn = url.openConnection();
		      conn.setConnectTimeout(30000);
		      BufferedReader reader = new BufferedReader(new InputStreamReader(conn.getInputStream()));
		      String line = null;
		    ...
		    if(str.indexOf("Total Assignments:")>0){
		    	 ...
		    	 //if(str.matches(".*Total Assignments:.+[0-9]+.*")){
		    		    //if(str.matches(".+Total Assignments:.*[0-9].+")){
		    		    	...
		    		    	str = document.substring(document.indexOf("Total Assignments:"), document.indexOf("Total Assignments:")+32).replace("\n", " ");
